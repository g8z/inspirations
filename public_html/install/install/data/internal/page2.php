...
// Changed by Swaroop(PHP Duo) Id: #1 v3.pd.1 Ref #1
// Installion error if temp file exists(as it's not chmodded to 777) for some reason
//if($fp = @fopen($files['smarty'].'temp','w')) {

$time=time();
if($fp = @fopen($files['smarty'].$time,'w')) {
  $page_str[] = array('str' => 'Smarty Compile Directory (/smarty/compile) probing..',
                      'res' => '[write test ok]');
  @unlink($files['smarty'].$time);
}else {
  $page_str[] = array('str' => 'Smarty Compile Directory (/smarty/compile) probing..',
                      'res' => '<span style="color:red">[write test failed]</span>');
  $proceed = false;
...
<table summary="" style="width:100%; height:100%" border="0" cellspacing="10">
  ...
  <tr>
    <td class="pane_desc">Wizard checks whether all neccessary files exist and respective permissions are set.</td>
  </tr>
  <tr>
    <td valign="top" style="padding:2px,0px,2px,10px; height:100%">
