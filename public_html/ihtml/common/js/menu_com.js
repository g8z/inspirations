...
function KeepPos(){
	...
	if(TS!=StaticPos){
		var FCStyle=Nav4?FrstCntnr:FrstCntnr.style;
		FCStyle.top=FrstCntnr.OrgTop=(StartTop+TS)+'px';StaticPos=TS}}
...
//	this.style.color=this.LwFntClr;
	...
	if(eval(WhatMenu+'[3]')&&ShowArrow){
		...
		if(DomYes){
			...
			t.style.top=T+'px';
			t.style.left=L+'px'}
		else{	MemVal+="<div style='position:absolute; top:"+T+"px; left:"+L+"px; width:"+W+"px; height:"+H+"px;visibility:inherit'><img src='"+S+"'></div>";
...
function NavMbrSetUp(MmbrCntnr,PrMmbr,WhatMenu,Wdth,Hght){
	...
	if(eval(WhatMenu+'[3]')&&ShowArrow){
		...
		this.CmdLyr.ImgLyr.top=RcrsLvl==1&&FirstLineHorizontal?(Hght-Arrws[a+2]-2)+'px':((Hght-Arrws[a+2])/2)+'px';
		this.CmdLyr.ImgLyr.left=RightToLeft?2+'px':(Wdth-Arrws[a+1]-2)+'px';
